...
from utils import get_intents_selection, get_filtered_lst, print_results, dataset_2_string, get_X_y_fasttext, \
    find_best_threshold, save_results, DS_INCOMPLETE_PATH, PRETRAINED_VECTORS_PATH
from tempfile import NamedTemporaryFile
from numpy import mean


def evaluate(dataset, dim: int, limit_num_sents: bool):
    train_str = dataset_2_string(dataset['train'], limit_num_sents=limit_num_sents, set_type='train')
    X_val, y_val = get_X_y_fasttext(dataset['val'] + dataset['oos_val'], limit_num_sents=limit_num_sents,
                                    set_type='val')
